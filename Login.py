...
def test_login_with_correct_username_and_wrong_password(driver):
    ...
    randomString = ''.join(random.choice(characters) for i in range(8))
    driver.find_element(
        By.NAME, 'username').send_keys(constant.USERNAME)
    driver.find_element(
        By.NAME, 'password').send_keys(randomString + Keys.ENTER)
    sleep(1)
...
def test_login_with_wrong_username_and_correct_password(driver):
    ...
    randomString = ''.join(random.choice(characters) for i in range(8))
    driver.find_element(
        By.NAME, 'username').send_keys(randomString)
    driver.find_element(
        By.NAME, 'password').send_keys(constant.PASSWORD + Keys.ENTER)
    sleep(1)